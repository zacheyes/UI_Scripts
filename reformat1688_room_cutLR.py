...
def process_image(file_path, output_dir, canvas_size=(3000, 1688), margin=0):
    ...
    try:
        ...
        # Compute scale to match canvas height (ignoring width, it may overflow)
        # Note: margin is 0 in this script, so content_h = canvas_h
        canvas_w, canvas_h = canvas_size
        content_h = canvas_h - 2 * margin # margin is 0, so this is just canvas_h
        
        crop_w, crop_h = cropped.size

        # Prevent division by zero if crop_h is 0
        if crop_h == 0:
            print(f"  ➔ Skipped (zero height after crop): {os.path.basename(file_path)}")
            return False

        scale = content_h / crop_h  # Scale ONLY by height
        ...
        paste_y = (canvas_h - new_h) // 2 # Should be 0 if scaled exactly to height and margin is 0
...
def main():
    parser = argparse.ArgumentParser(description="Process and crop room images to 3000x1688, scaling by height (Cut LR version).")
    ...
    if not args.input:
        print("\n--- Running Room Cropping Script (3000x1688 Cut LR) in Standalone Mode ---")
    ...
    output_dir = os.path.join(input_folder, "Processed_1688_Room_CutLR")
